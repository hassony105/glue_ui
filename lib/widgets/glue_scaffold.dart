...
/// Manages [SnackBar]s and [MaterialBanner]s for descendant [GlueScaffold]s.
///
...
///
/// To display one of these notifications, obtain the [GlueScaffoldMessengerState]
/// for the current [BuildContext] via [GlueScaffoldMessenger.of] and use the
/// [GlueScaffoldMessengerState.showSnackBar] or the
/// [GlueScaffoldMessengerState.showMaterialBanner] functions.
///
/// When the [GlueScaffoldMessenger] has nested [GlueScaffold] descendants, the
...
///    bottom of the app using the [GlueScaffoldMessengerState.showSnackBar] method.
///  * [MaterialBanner], which is a temporary notification typically shown at the
///    top of the app using the [GlueScaffoldMessengerState.showMaterialBanner] method.
///  * [debugCheckHasScaffoldMessenger], which asserts that the given context
///    has a [GlueScaffoldMessenger] ancestor.
///  * Cookbook: [Display a SnackBar](https://docs.flutter.dev/cookbook/design/snackbars)
class GlueScaffoldMessenger extends StatefulWidget {
  /// Creates a widget that manages [SnackBar]s for [GlueScaffold] descendants.
  const GlueScaffoldMessenger({super.key, required this.child});
  ...
  ///
  /// {@tool dartpad}
  /// Typical usage of the [GlueScaffoldMessenger.of] function is to call it in
  ...
  /// [GlueScaffoldMessenger], then use the `key.currentState` property to obtain the
  /// [GlueScaffoldMessengerState] rather than using the [GlueScaffoldMessenger.of]
  ...
  /// cases, you can assign a [GlobalKey] to the [GlueScaffoldMessenger]. This
  /// example shows a key being used to obtain the [GlueScaffoldMessengerState]
  /// provided by the [MaterialApp].
  ///
  /// ** See code in examples/api/lib/material/scaffold/scaffold_messenger.of.1.dart **
  /// {@end-tool}
  ///
  /// If there is no [GlueScaffoldMessenger] in scope, then this will assert in
  /// debug mode, and throw an exception in release mode.
  ///
  /// See also:
  ///
  ///  * [maybeOf], which is a similar function but will return null instead of
  ///    throwing if there is no [GlueScaffoldMessenger] ancestor.
  ///  * [debugCheckHasScaffoldMessenger], which asserts that the given context
  ///    has a [GlueScaffoldMessenger] ancestor.
  static GlueScaffoldMessengerState of(BuildContext context) {
  ...
  /// context, if any.
  ///
  /// Will return null if a [GlueScaffoldMessenger] is not found in the given context.
  ///
  /// See also:
  ///
  ///  * [of], which is a similar function, except that it will throw an
  ///    exception if a [GlueScaffoldMessenger] is not found in the given context.
  static GlueScaffoldMessengerState? maybeOf(BuildContext context) {
    final _ScaffoldMessengerScope? scope =
    context.dependOnInheritedWidgetOfExactType<_ScaffoldMessengerScope>();
    return scope?._scaffoldMessengerState;
  }
  ...
  @override
  GlueScaffoldMessengerState createState() => GlueScaffoldMessengerState();
}

/// State for a [GlueScaffoldMessenger].
///
/// A [GlueScaffoldMessengerState] object can be used to [showSnackBar] or
/// [showMaterialBanner] for every registered [GlueScaffold] that is a descendant of
/// the associated [GlueScaffoldMessenger]. Scaffolds will register to receive
/// [SnackBar]s and [MaterialBanner]s from their closest ScaffoldMessenger
/// ancestor.
///
/// Typically obtained via [GlueScaffoldMessenger.of].
class GlueScaffoldMessengerState extends State<GlueScaffoldMessenger> with TickerProviderStateMixin {
  final LinkedHashSet<GlueScaffoldState> _scaffolds = LinkedHashSet<GlueScaffoldState>();
  final Queue<GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason>>
  _materialBanners = Queue<GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason>>();
  AnimationController? _materialBannerController;
  final Queue<GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason>> _snackBars =
  Queue<GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason>>();
  ...
  void _register(GlueScaffoldState scaffold) {
  ...
  void _unregister(GlueScaffoldState scaffold) {
  ...
  void _updateScaffolds() {
    for (final GlueScaffoldState scaffold in _scaffolds) {
      if (_isRoot(scaffold)) {
        scaffold._updateSnackBar();
        scaffold._updateMaterialBanner();
      }
  ...
  bool _isRoot(GlueScaffoldState scaffold) {
    final GlueScaffoldState? parent = scaffold.context.findAncestorStateOfType<GlueScaffoldState>();
    return parent == null || !_scaffolds.contains(parent);
  }

  // SNACKBAR API

  /// Shows a [SnackBar] across all registered [GlueScaffold]s. Scaffolds register
  /// to receive snack bars from their closest [GlueScaffoldMessenger] ancestor.
  ...
  /// or call [GlueScaffoldFeatureController.close] on the returned
  /// [GlueScaffoldFeatureController]. To remove a [SnackBar] suddenly (without an
  /// animation), use [removeCurrentSnackBar].
  ///
  /// See [GlueScaffoldMessenger.of] for information about how to obtain the
  /// [GlueScaffoldMessengerState].
  ///
  /// {@tool dartpad}
  /// Here is an example of showing a [SnackBar] when the user presses a button.
  ///
  ...
  /// positioned above the widgets provided to [GlueScaffold.floatingActionButton],
  /// [GlueScaffold.persistentFooterButtons], and [GlueScaffold.bottomNavigationBar].
  ...
  ///
  /// {@tool dartpad}
  /// Here is an example showing that a floating [SnackBar] appears above [GlueScaffold.floatingActionButton].
  ///
  ...
  /// duration using [AnimationStyle] in [GlueScaffoldMessengerState.showSnackBar].
  ///
  /// ** See code in examples/api/lib/material/scaffold/scaffold_messenger_state.show_snack_bar.2.dart **
  /// {@end-tool}
  ///
  GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason> showSnackBar(
      SnackBar snackBar, {
    ...
    late GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason> controller;
    controller = GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason>._(
  ...
  /// Shows a [MaterialBanner] across all registered [GlueScaffold]s. Scaffolds register
  /// to receive material banners from their closest [GlueScaffoldMessenger] ancestor.
  ...
  /// or call [GlueScaffoldFeatureController.close] on the returned
  /// [GlueScaffoldFeatureController]. To remove a [MaterialBanner] suddenly (without an
  ...
  /// {@end-tool}
  GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason> showMaterialBanner(
    ...
    late GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason> controller;
    controller = GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason>._(
  ...
  void clearMaterialBanners() {
    ...
    final GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason>
...
class _ScaffoldMessengerScope extends InheritedWidget {
  const _ScaffoldMessengerScope({
    ...
    required GlueScaffoldMessengerState scaffoldMessengerState,
  }) : _scaffoldMessengerState = scaffoldMessengerState;

  final GlueScaffoldMessengerState _scaffoldMessengerState;
...
/// The geometry of the [GlueScaffold] after all its contents have been laid out
/// except the [FloatingActionButton].
///
/// The [GlueScaffold] passes this pre-layout geometry to its
/// [FloatingActionButtonLocation], which produces an [Offset] that the
/// [GlueScaffold] uses to position the [FloatingActionButton].
///
/// For a description of the [GlueScaffold]'s geometry after it has
/// finished laying out, see the [GlueScaffoldGeometry].
@immutable
class GlueScaffoldPrelayoutGeometry extends ScaffoldPrelayoutGeometry{
  /// Abstract const constructor. This constructor enables subclasses to provide
  /// const constructors so that they can be used in const expressions.
  const GlueScaffoldPrelayoutGeometry({
...
///
/// [GlueScaffoldState] uses this to seamlessly change transition animations
/// when a running [FloatingActionButtonLocation] transition is interrupted by a new transition.
@immutable
class _GlueTransitionSnapshotFabLocation extends FloatingActionButtonLocation {
  const _GlueTransitionSnapshotFabLocation(this.begin, this.end, this.animator, this.progress);
...
/// Geometry information for [GlueScaffold] components after layout is finished.
///
/// To get a [ValueNotifier] for the scaffold geometry of a given
/// [BuildContext], use [GlueScaffold.geometryOf].
///
/// The ScaffoldGeometry is only available during the paint phase, because
/// its value is computed during the animation and layout phases prior to painting.
///
/// For an example of using the [GlueScaffoldGeometry], see the [BottomAppBar],
/// which uses the [GlueScaffoldGeometry] to paint a notch around the
/// [FloatingActionButton].
///
/// For information about the [GlueScaffold]'s geometry that is used while laying
/// out the [FloatingActionButton], see [GlueScaffoldPrelayoutGeometry].
@immutable
class GlueScaffoldGeometry {
  /// Create an object that describes the geometry of a [GlueScaffold].
  const GlueScaffoldGeometry({this.bottomNavigationBarTop, this.floatingActionButtonArea});

  /// The distance from the [GlueScaffold]'s top edge to the top edge of the
  /// rectangle in which the [GlueScaffold.bottomNavigationBar] bar is laid out.
  ///
  /// Null if [GlueScaffold.bottomNavigationBar] is null.
  final double? bottomNavigationBarTop;

  /// The [GlueScaffold.floatingActionButton]'s bounding rectangle.
  ///
  /// This is null when there is no floating action button showing.
  final Rect? floatingActionButtonArea;

  GlueScaffoldGeometry _scaleFloatingActionButton(double scaleFactor) {
    if (scaleFactor == 1.0) {
      return this;
    }

    if (scaleFactor == 0.0) {
      return GlueScaffoldGeometry(bottomNavigationBarTop: bottomNavigationBarTop);
    }
  ...
  /// Creates a copy of this [GlueScaffoldGeometry] but with the given fields replaced with
  /// the new values.
  GlueScaffoldGeometry copyWith({double? bottomNavigationBarTop, Rect? floatingActionButtonArea}) {
    return GlueScaffoldGeometry(
...
class _GlueScaffoldGeometryNotifier extends ChangeNotifier
    implements ValueListenable<GlueScaffoldGeometry> {
  _GlueScaffoldGeometryNotifier(this.geometry, this.context);

  final BuildContext context;
  double? floatingActionButtonScale;
  GlueScaffoldGeometry geometry;

  @override
  GlueScaffoldGeometry get value {
    assert(() {
...
class _GlueBodyBoxConstraints extends BoxConstraints {
  const _GlueBodyBoxConstraints({
    super.maxWidth,
    super.maxHeight,
  ...
  bool operator ==(Object other) {
    ...
    return other is _GlueBodyBoxConstraints &&
...
class _GlueBodyBuilder extends StatelessWidget {
  const _GlueBodyBuilder({
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        final _GlueBodyBoxConstraints bodyConstraints = constraints as _GlueBodyBoxConstraints;
...
class _GlueScaffoldLayout extends MultiChildLayoutDelegate {
  _GlueScaffoldLayout({
  ...
  final _GlueScaffoldGeometryNotifier geometryNotifier;
  ...
  void performLayout(Size size) {
    ...
    if (hasChild(_GlueScaffoldSlot.appBar)) {
      appBarHeight = layoutChild(_GlueScaffoldSlot.appBar, fullWidthConstraints).height;
      contentTop = extendBodyBehindAppBar ? 0.0 : appBarHeight;
      positionChild(_GlueScaffoldSlot.appBar, Offset.zero);
    }

    double? bottomNavigationBarTop;
    if (hasChild(_GlueScaffoldSlot.bottomNavigationBar)) {
      final double bottomNavigationBarHeight =
          layoutChild(_GlueScaffoldSlot.bottomNavigationBar, fullWidthConstraints).height;
      bottomWidgetsHeight += bottomNavigationBarHeight;
      bottomNavigationBarTop = math.max(0.0, bottom - bottomWidgetsHeight);
      positionChild(_GlueScaffoldSlot.bottomNavigationBar, Offset(0.0, bottomNavigationBarTop));
    }

    if (hasChild(_GlueScaffoldSlot.persistentFooter)) {
      ...
      final double persistentFooterHeight =
          layoutChild(_GlueScaffoldSlot.persistentFooter, footerConstraints).height;
      bottomWidgetsHeight += persistentFooterHeight;
      positionChild(
        _GlueScaffoldSlot.persistentFooter,
        Offset(0.0, math.max(0.0, bottom - bottomWidgetsHeight)),
      );
    }

    Size materialBannerSize = Size.zero;
    if (hasChild(_GlueScaffoldSlot.materialBanner)) {
      materialBannerSize = layoutChild(_GlueScaffoldSlot.materialBanner, fullWidthConstraints);
      positionChild(_GlueScaffoldSlot.materialBanner, Offset(0.0, appBarHeight));
    ...
    if (hasChild(_GlueScaffoldSlot.body)) {
      ...
      final BoxConstraints bodyConstraints = _GlueBodyBoxConstraints(
        maxWidth: fullWidthConstraints.maxWidth,
      ...
      layoutChild(_GlueScaffoldSlot.body, bodyConstraints);
      positionChild(_GlueScaffoldSlot.body, Offset(0.0, contentTop));
    }
    ...
    if (hasChild(_GlueScaffoldSlot.bodyScrim)) {
      final BoxConstraints bottomSheetScrimConstraints = BoxConstraints(
        maxWidth: fullWidthConstraints.maxWidth,
        maxHeight: contentBottom,
      );
      layoutChild(_GlueScaffoldSlot.bodyScrim, bottomSheetScrimConstraints);
      positionChild(_GlueScaffoldSlot.bodyScrim, Offset.zero);
    }

    // Set the size of the SnackBar early if the behavior is fixed so
    // the FAB can be positioned correctly.
    if (hasChild(_GlueScaffoldSlot.snackBar) && !isSnackBarFloating) {
      snackBarSize = layoutChild(_GlueScaffoldSlot.snackBar, fullWidthConstraints);
    }

    if (hasChild(_GlueScaffoldSlot.bottomSheet)) {
      final BoxConstraints bottomSheetConstraints = BoxConstraints(
        maxWidth: fullWidthConstraints.maxWidth,
        maxHeight: math.max(0.0, contentBottom - contentTop),
      );
      bottomSheetSize = layoutChild(_GlueScaffoldSlot.bottomSheet, bottomSheetConstraints);
      positionChild(
        _GlueScaffoldSlot.bottomSheet,
        Offset((size.width - bottomSheetSize.width) / 2.0, contentBottom - bottomSheetSize.height),
      );
    }

    late Rect floatingActionButtonRect;
    if (hasChild(_GlueScaffoldSlot.floatingActionButton)) {
      final Size fabSize = layoutChild(_GlueScaffoldSlot.floatingActionButton, looseConstraints);

      // To account for the FAB position being changed, we'll animate between
      // the old and new positions.
      final GlueScaffoldPrelayoutGeometry currentGeometry = GlueScaffoldPrelayoutGeometry(
      ...
      positionChild(_GlueScaffoldSlot.floatingActionButton, fabOffset);
      floatingActionButtonRect = fabOffset & fabSize;
    }

    if (hasChild(_GlueScaffoldSlot.snackBar)) {
      final bool hasCustomWidth = snackBarWidth != null && snackBarWidth! < size.width;
      if (snackBarSize == Size.zero) {
        snackBarSize = layoutChild(
          _GlueScaffoldSlot.snackBar,
    ...
    if (hasChild(_GlueScaffoldSlot.statusBar)) {
      layoutChild(_GlueScaffoldSlot.statusBar, fullWidthConstraints.tighten(height: minInsets.top));
      positionChild(_GlueScaffoldSlot.statusBar, Offset.zero);
    }

    if (hasChild(_GlueScaffoldSlot.drawer)) {
      layoutChild(_GlueScaffoldSlot.drawer, BoxConstraints.tight(size));
      positionChild(_GlueScaffoldSlot.drawer, Offset.zero);
    }

    if (hasChild(_GlueScaffoldSlot.endDrawer)) {
      layoutChild(_GlueScaffoldSlot.endDrawer, BoxConstraints.tight(size));
      positionChild(_GlueScaffoldSlot.endDrawer, Offset.zero);
    }
  ...
  @override
  bool shouldRelayout(_GlueScaffoldLayout oldDelegate) {
...
/// * Motion animations, which are triggered by the [GlueScaffold]
///   when its [FloatingActionButtonLocation] is updated.
class _GlueFloatingActionButtonTransition extends StatefulWidget {
  const _GlueFloatingActionButtonTransition({
    required this.child,
  ...
  @override
  _GlueFloatingActionButtonTransitionState createState() => _GlueFloatingActionButtonTransitionState();
}

class _GlueFloatingActionButtonTransitionState extends State<_GlueFloatingActionButtonTransition>
  ...
  @override
  void didUpdateWidget(_GlueFloatingActionButtonTransition oldWidget) {
...
/// [GlueScaffoldState] for the current [BuildContext] via [GlueScaffold.of] and use the
/// [GlueScaffoldState.showBottomSheet] function.
///
/// {@tool dartpad}
/// This example shows a [GlueScaffold] with a [body] and [FloatingActionButton].
/// The [body] is a [Text] placed in a [Center] in order to center the text
/// within the [GlueScaffold]. The [FloatingActionButton] is connected to a
...
///
/// {@tool dartpad}
/// This example shows a [GlueScaffold] with a blueGrey [backgroundColor], [body]
/// and [FloatingActionButton]. The [body] is a [Text] placed in a [Center] in
/// order to center the text within the [GlueScaffold]. The [FloatingActionButton]
...
///
/// {@tool dartpad}
/// This example shows a [GlueScaffold] with an [AppBar], a [BottomAppBar] and a
/// [FloatingActionButton]. The [body] is a [Text] placed in a [Center] in order
/// to center the text within the [GlueScaffold]. The [FloatingActionButton] is
...
///
/// If [GlueScaffold.bottomSheet] is a [DraggableScrollableSheet],
/// [GlueScaffold.floatingActionButton] is set, and the bottom sheet is dragged to
/// cover greater than 70% of the Scaffold's height, two things happen in parallel:
///
///   * Scaffold starts to show scrim (see [GlueScaffoldState.showBodyScrim]), and
///   * [GlueScaffold.floatingActionButton] is scaled down through an animation with a [Curves.easeIn], and
///     disappears when the bottom sheet covers the entire Scaffold.
///
/// And as soon as the bottom sheet is dragged down to cover less than 70% of the [GlueScaffold], the scrim
/// disappears and [GlueScaffold.floatingActionButton] animates back to its normal size.
///
/// ## Troubleshooting
///
/// ### Nested Scaffolds
...
///    using the [GlueScaffoldState.showBottomSheet] method, or modal, in which case
...
///    [GlueScaffoldMessengerState.showSnackBar] method to show snack bars.
///  * [MaterialBanner], which displays an important, succinct message, at the
///    top of the screen, below the app bar. Use the
///    [GlueScaffoldMessengerState.showMaterialBanner] method to show material banners.
///  * [GlueScaffoldState], which is the state associated with this widget.
///  * <https://material.io/design/layout/responsive-layout-grid.html>
///  * Cookbook: [Add a Drawer to a screen](https://docs.flutter.dev/cookbook/design/drawer)
class GlueScaffold extends StatefulWidget {
  ...
  ///
  /// If null, the [GlueScaffoldState] will use the default location, [FloatingActionButtonLocation.endFloat].
  final FloatingActionButtonLocation? floatingActionButtonLocation;

  /// Animator to move the [floatingActionButton] to a new [floatingActionButtonLocation].
  ///
  /// If null, the [GlueScaffoldState] will use the default animator, [FloatingActionButtonAnimator.scaling].
  ...
  /// Typically a [Drawer].
  ///
  /// To open the drawer, use the [GlueScaffoldState.openDrawer] function.
  ///
  /// To close the drawer, use either [GlueScaffoldState.closeDrawer], [Navigator.pop]
  /// or press the escape key on the keyboard.
  ///
  /// {@tool dartpad}
  /// To disable the drawer edge swipe on mobile, set the
  /// [GlueScaffold.drawerEnableOpenDragGesture] to false. Then, use
  /// [GlueScaffoldState.openDrawer] to open the drawer and [Navigator.pop] to close
  /// it.
  ///
  ...
  /// Optional callback that is called when the [GlueScaffold.drawer] is opened or closed.
  ...
  /// Typically a [Drawer].
  ///
  /// To open the drawer, use the [GlueScaffoldState.openEndDrawer] function.
  ///
  /// To close the drawer, use either [GlueScaffoldState.closeEndDrawer], [Navigator.pop]
  /// or press the escape key on the keyboard.
  ///
  /// {@tool dartpad}
  /// To disable the drawer edge swipe, set the
  /// [GlueScaffold.endDrawerEnableOpenDragGesture] to false. Then, use
  /// [GlueScaffoldState.openEndDrawer] to open the drawer and [Navigator.pop] to
  /// close it.
  ///
  ...
  /// Optional callback that is called when the [GlueScaffold.endDrawer] is opened or closed.
  ...
  /// Determines if the [GlueScaffold.drawer] can be opened with a drag
  /// gesture on mobile.
  ///
  ...
  /// Determines if the [GlueScaffold.endDrawer] can be opened with a
  /// gesture on mobile.
  ///
  /// On desktop platforms, the drawer is not draggable.
  ///
  ...
  /// Restoration ID to save and restore the state of the [GlueScaffold].
  ///
  ...
  /// Finds the [GlueScaffoldState] from the closest instance of this class that
  ...
  ///
  /// {@tool dartpad}
  /// Typical usage of the [GlueScaffold.of] function is to call it from within the
  /// `build` method of a child of a [GlueScaffold].
  ///
  /// ** See code in examples/api/lib/material/scaffold/scaffold.of.0.dart **
  /// {@end-tool}
  ///
  /// {@tool dartpad}
  /// When the [GlueScaffold] is actually created in the same `build` function, the
  /// `context` argument to the `build` function can't be used to find the
  /// [GlueScaffold] (since it's "above" the widget being returned in the widget
  ...
  /// [GlueScaffold]. In this solution, you would have an outer widget that creates
  /// the [GlueScaffold] populated by instances of your new inner widgets, and then
  /// in these inner widgets you would use [GlueScaffold.of].
  ///
  /// A less elegant but more expedient solution is assign a [GlobalKey] to the
  /// [GlueScaffold], then use the `key.currentState` property to obtain the
  /// [GlueScaffoldState] rather than using the [GlueScaffold.of] function.
  ///
  /// If there is no [GlueScaffold] in scope, then this will throw an exception.
  /// To return null if there is no [GlueScaffold], use [maybeOf] instead.
  static GlueScaffoldState of(BuildContext context) {
    final GlueScaffoldState? result = context.findAncestorStateOfType<GlueScaffoldState>();
    if (result != null) {
      return result;
    }
    throw FlutterError.fromParts(<DiagnosticsNode>[
  ...
  ///    documentation.
  static GlueScaffoldState? maybeOf(BuildContext context) {
    return context.findAncestorStateOfType<GlueScaffoldState>();
  }

  /// Returns a [ValueListenable] for the [GlueScaffoldGeometry] for the closest
  /// [GlueScaffold] ancestor of the given context.
  ///
  ...
  /// paint pass.
  ///
  /// The closest [GlueScaffold] ancestor for the context might change, e.g when
  /// an element is moved from one scaffold to another. For [StatefulWidget]s
  /// using this listenable, a change of the [GlueScaffold] ancestor will
  /// trigger a [State.didChangeDependencies].
  ///
  /// A typical pattern for listening to the scaffold geometry would be to
  /// call [GlueScaffold.geometryOf] in [State.didChangeDependencies], compare the
  /// return value with the previous listenable, if it has changed, unregister
  /// the listener, and register a listener to the new [GlueScaffoldGeometry]
  /// listenable.
  static ValueListenable<GlueScaffoldGeometry> geometryOf(BuildContext context) {
    final _GlueScaffoldScope? scaffoldScope =
    context.dependOnInheritedWidgetOfExactType<_GlueScaffoldScope>();
  ...
  /// [GlueScaffold] so that the client widget gets rebuilt whenever the [hasDrawer]
  /// value changes.
  ///
  /// This method can be expensive (it walks the element tree).
  ///
  /// See also:
  ///
  ///  * [GlueScaffold.of], which provides access to the [GlueScaffoldState] object as a
  ///    whole, from which you can show bottom sheets, and so forth.
  static bool hasDrawer(BuildContext context, {bool registerForUpdates = true}) {
    if (registerForUpdates) {
      final _GlueScaffoldScope? scaffold = context.dependOnInheritedWidgetOfExactType<_GlueScaffoldScope>();
      return scaffold?.hasDrawer ?? false;
    } else {
      final GlueScaffoldState? scaffold = context.findAncestorStateOfType<GlueScaffoldState>();
      return scaffold?.hasDrawer ?? false;
    }
  }

  @override
  GlueScaffoldState createState() => GlueScaffoldState();
}

/// State for a [GlueScaffold].
///
/// Can display [BottomSheet]s. Retrieve a [GlueScaffoldState] from the current
/// [BuildContext] using [GlueScaffold.of].
class GlueScaffoldState extends State<GlueScaffold> with TickerProviderStateMixin, RestorationMixin {
  @override
  ...
  /// Whether this scaffold has a non-null [GlueScaffold.appBar].
  bool get hasAppBar => widget.appBar != null;

  /// Whether this scaffold has a non-null [GlueScaffold.drawer].
  bool get hasDrawer => widget.drawer != null;

  /// Whether this scaffold has a non-null [GlueScaffold.endDrawer].
  bool get hasEndDrawer => widget.endDrawer != null;

  /// Whether this scaffold has a non-null [GlueScaffold.floatingActionButton].
  bool get hasFloatingActionButton => widget.floatingActionButton != null;

  double? _appBarMaxHeight;

  /// The max height the [GlueScaffold.appBar] uses.
  ///
  ...
  /// Whether the [GlueScaffold.drawer] is opened.
  ///
  /// See also:
  ///
  ///  * [GlueScaffoldState.openDrawer], which opens the [GlueScaffold.drawer] of a
  ///    [GlueScaffold].
  bool get isDrawerOpen => _drawerOpened.value;

  /// Whether the [GlueScaffold.endDrawer] is opened.
  ///
  /// See also:
  ///
  ///  * [GlueScaffoldState.openEndDrawer], which opens the [GlueScaffold.endDrawer] of
  ///    a [GlueScaffold].
  ...
  ///
  /// If the scaffold has a non-null [GlueScaffold.drawer], this function will cause
  /// the drawer to begin its entrance animation.
  ///
  /// Normally this is not needed since the [GlueScaffold] automatically shows an
  /// appropriate [IconButton], and handles the edge-swipe gesture, to show the
  /// drawer.
  ///
  /// To close the drawer, use either [GlueScaffoldState.closeDrawer] or
  /// [Navigator.pop].
  ///
  /// See [GlueScaffold.of] for information about how to obtain the [GlueScaffoldState].
  void openDrawer() {
  ...
  ///
  /// If the scaffold has a non-null [GlueScaffold.endDrawer], this function will cause
  ...
  /// drawer.
  ///
  /// To close the drawer, use either [GlueScaffoldState.closeEndDrawer] or
  /// [Navigator.pop].
  ///
  /// See [GlueScaffold.of] for information about how to obtain the [GlueScaffoldState].
  void openEndDrawer() {
  ...
  GlueScaffoldMessengerState? _scaffoldMessenger;

  // SNACKBAR API
  GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason>? _messengerSnackBar;

  // This is used to update the _messengerSnackBar by the ScaffoldMessenger.
  void _updateSnackBar() {
    final GlueScaffoldFeatureController<SnackBar, SnackBarClosedReason>? messengerSnackBar =
  ...
  GlueScaffoldFeatureController<MaterialBanner, MaterialBannerClosedReason>? _messengerMaterialBanner;
  ...
  // bottom sheet.
  final List<_GlueStandardBottomSheet> _dismissedBottomSheets = <_GlueStandardBottomSheet>[];
  GluePersistentBottomSheetController? _currentBottomSheet;
  ...
  void _maybeBuildPersistentBottomSheet() {
    if (widget.bottomSheet != null && _currentBottomSheet == null) {
      ...
      if (_dismissedBottomSheets.isNotEmpty) {
        final List<_GlueStandardBottomSheet> sheets = List<_GlueStandardBottomSheet>.of(
          _dismissedBottomSheets,
          growable: false,
        );
        for (final _GlueStandardBottomSheet sheet in sheets) {
  ...
  /// Closes [GlueScaffold.drawer] if it is currently opened.
  ///
  /// See [GlueScaffold.of] for information about how to obtain the [GlueScaffoldState].
  void closeDrawer() {
  ...
  /// Closes [GlueScaffold.endDrawer] if it is currently opened.
  ///
  /// See [GlueScaffold.of] for information about how to obtain the [GlueScaffoldState].
  void closeEndDrawer() {
  ...
  GluePersistentBottomSheetController _buildBottomSheet(
    ...
    final GlobalKey<_GlueStandardBottomSheetState> bottomSheetKey =
    GlobalKey<_GlueStandardBottomSheetState>();
    late _GlueStandardBottomSheet bottomSheet;
    ...
    bottomSheet = _GlueStandardBottomSheet(
      key: bottomSheetKey,
      animationController: animationController,
    ...
    return GluePersistentBottomSheetController._(
      bottomSheet,
      completer,
  ...
  /// Shows a Material Design bottom sheet in the nearest [GlueScaffold]. To show
  /// a persistent bottom sheet, use the [GlueScaffold.bottomSheet].
  ///
  /// Returns a controller that can be used to close and otherwise manipulate the
  /// bottom sheet.
  ///
  /// To rebuild the bottom sheet (e.g. if it is stateful), call
  /// [GluePersistentBottomSheetController.setState] on the controller returned by
  /// this method.
  ///
  /// The new bottom sheet becomes a [LocalHistoryEntry] for the enclosing
  /// [ModalRoute] and a back button is added to the app bar of the [GlueScaffold]
  ...
  /// [GlueScaffold.bottomSheet] constructor parameter.
  ///
  /// A persistent bottom sheet shows information that supplements the primary
  /// content of the app. A persistent bottom sheet remains visible even when
  /// the user interacts with other parts of the app.
  ...
  ///    sheet.
  ///  * [GlueScaffold.of], for information about how to obtain the [GlueScaffoldState].
  ...
  ///    animation duration and reverse animation duration.
  GluePersistentBottomSheetController showBottomSheet(
      WidgetBuilder builder, {
        Color? backgroundColor,
        double? elevation,
        ShapeBorder? shape,
  ...
  /// [GlueScaffold.floatingActionButton].
  ...
  /// Shows the [GlueScaffold.floatingActionButton].
  ...
  void _moveFloatingActionButton(final FloatingActionButtonLocation newLocation) {
    ...
    if (_floatingActionButtonMoveController.isAnimating) {
      previousLocation = _GlueTransitionSnapshotFabLocation(
  ...
  late _GlueScaffoldGeometryNotifier _geometryNotifier;
  ...
  void initState() {
    super.initState();
    _geometryNotifier = _GlueScaffoldGeometryNotifier(const GlueScaffoldGeometry(), context);
    _floatingActionButtonLocation =
  ...
  @protected
  @override
  void didUpdateWidget(GlueScaffold oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  void didChangeDependencies() {
    ...
    final GlueScaffoldMessengerState? currentScaffoldMessenger = GlueScaffoldMessenger.maybeOf(context);
  ...
  Widget build(BuildContext context) {
    ...
    if (_messengerMaterialBanner != null) {
      ...
      _addIfNonNull(
        ...
        _GlueScaffoldSlot.materialBanner,
        removeLeftPadding: false,
        removeTopPadding: widget.appBar != null,
        removeRightPadding: false,
        removeBottomPadding: true,
    ...
    if (widget.bottomNavigationBar != null) {
      _addIfNonNull(
        ...
        _GlueScaffoldSlot.bottomNavigationBar,
        removeLeftPadding: false,
        removeTopPadding: true,
        removeRightPadding: false,
        removeBottomPadding: false,
    ...
    _addIfNonNull(
      children,
      _GlueFloatingActionButtonTransition(
      ...
      _GlueScaffoldSlot.floatingActionButton,
      removeLeftPadding: true,
      removeTopPadding: true,
      removeRightPadding: true,
      removeBottomPadding: true,
        ...
        child: Material(
          ...
          child: AnimatedBuilder(
            ...
            builder: (BuildContext context, Widget? child) {
              return Actions(
                actions: <Type, Action<Intent>>{DismissIntent: _GlueDismissDrawerAction(context)},
                child: CustomMultiChildLayout(
                  delegate: _GlueScaffoldLayout(
                    extendBody: widget.extendBody,
                    extendBodyBehindAppBar: widget.extendBodyBehindAppBar,
                    minInsets: minInsets,
                    minViewPadding: minViewPadding,
...
class _GlueDismissDrawerAction extends DismissAction {
  _GlueDismissDrawerAction(this.context);

  final BuildContext context;

  @override
  bool isEnabled(DismissIntent intent) {
    return GlueScaffold.of(context).isDrawerOpen || GlueScaffold.of(context).isEndDrawerOpen;
  }

  @override
  void invoke(DismissIntent intent) {
    GlueScaffold.of(context).closeDrawer();
    GlueScaffold.of(context).closeEndDrawer();
  }
}

/// An interface for controlling a feature of a [GlueScaffold].
///
/// Commonly obtained from [GlueScaffoldMessengerState.showSnackBar] or
/// [GlueScaffoldState.showBottomSheet].
class GlueScaffoldFeatureController<T extends Widget, U> {
  const GlueScaffoldFeatureController._(this._widget, this._completer, this.close, this.setState);
  final T _widget;
...
class _GlueStandardBottomSheet extends StatefulWidget {
  const _GlueStandardBottomSheet({
    super.key,
  ...
  @override
  _GlueStandardBottomSheetState createState() => _GlueStandardBottomSheetState();
}

class _GlueStandardBottomSheetState extends State<_GlueStandardBottomSheet> {
  ...
  @override
  void didUpdateWidget(_GlueStandardBottomSheet oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  bool extentChanged(DraggableScrollableNotification notification) {
    ...
    final GlueScaffoldState scaffold = GlueScaffold.of(context);
...
/// A [GlueScaffoldFeatureController] for standard bottom sheets.
///
/// This is the type of objects returned by [GlueScaffoldState.showBottomSheet].
///
...
/// [GlueScaffold.bottomSheet].
class GluePersistentBottomSheetController
    extends GlueScaffoldFeatureController<_GlueStandardBottomSheet, void> {
  const GluePersistentBottomSheetController._(
      super.widget,
      super.completer,
      super.close,
...
class _GlueScaffoldScope extends InheritedWidget {
  const _GlueScaffoldScope({
  ...
  @override
  bool updateShouldNotify(_GlueScaffoldScope oldWidget) {
